export const LEADERBOARD_LIMIT = 1_000;
export const LEADERBOARD_TOP_COUNT = 5;
export const LEADERBOARD_CONTEXT_RADIUS = 2;
...
export function buildLeaderboardWindow(
  entries,
  playerRank = null,
  topCount = LEADERBOARD_TOP_COUNT,
  contextRadius = LEADERBOARD_CONTEXT_RADIUS
) {
  const rankedEntries = Array.isArray(entries) ? entries : [];
  const selectedRanks = new Set();
  const safeTopCount = Math.max(0, Math.floor(topCount));
  const safeContextRadius = Math.max(0, Math.floor(contextRadius));

  for (let rank = 1; rank <= Math.min(safeTopCount, rankedEntries.length); rank += 1) {
    selectedRanks.add(rank);
  }

  if (Number.isInteger(playerRank) && playerRank >= 1 && playerRank <= rankedEntries.length) {
    const firstContextRank = Math.max(1, playerRank - safeContextRadius);
    const lastContextRank = Math.min(rankedEntries.length, playerRank + safeContextRadius);
    for (let rank = firstContextRank; rank <= lastContextRank; rank += 1) {
      selectedRanks.add(rank);
    }
  }
  ...
  return [...selectedRanks]
    ...
    .map((rank) => ({ ...rankedEntries[rank - 1], rank }));
}

export function emptyLeaderboardDocument() {
  return { version: 3, normal: [], zen: [] };
}
