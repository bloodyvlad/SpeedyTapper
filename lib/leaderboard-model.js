...
export function normalizeScoreSubmission(input, metadata = {}) {
  ...
  const minimumPlausibleScore = hits * MIN_POINTS_PER_HIT;
  const maximumPlausibleScore = hits * MAX_POINTS_PER_HIT;
  if (score < minimumPlausibleScore || score > maximumPlausibleScore) {
    throw new LeaderboardValidationError("Score does not match the tap count.");
  }
