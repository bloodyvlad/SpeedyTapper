...
container="pimpopom-multiplayer-v2-mariadb-$$"
cleanup() {
...
docker run -d --name "$container" -e MARIADB_ROOT_PASSWORD=root \
    -e MARIADB_DATABASE=speedytapper -p 127.0.0.1::3306 mariadb:11.4 >/dev/null
...
SPEEDYTAPPER_TEST_MARIADB_USER=root SPEEDYTAPPER_TEST_MARIADB_PASSWORD=root \
php "$(dirname "$0")/multiplayer-v2-auth.test.php"
