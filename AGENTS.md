...
- Google and Apple identity tokens are verified server-side and map provider-domain-separated subject digests to one internal UUID. A second primary provider is attached only through explicit recent-authenticated linking; never merge profiles based on email, relay email, nickname, device, StoreKit, or Game Center. Game Center is a verified link-only secondary identity and cannot register or log in. Require an explicitly confirmed public nickname before score submission; never import or publish provider display names. Store only the internal UUID, public nickname, confirmation flag, one-way provider-subject digests, the hashed Game Center team identity, and the Apple refresh token as authenticated ciphertext required for deletion-time revocation—never raw subjects, identity tokens, Game Center IDs, email addresses, or passwords.
