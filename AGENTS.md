...
- Godlike and Perfect taps advance a five-step boost meter. Every five qualifying taps unlocks the next multiplier for subsequent taps, up to 5×. Great is neutral; Good and every mistake reset the boost to 1×; dodges are neutral and unmultiplied. Apply a multiplier only to the current tap award—never retroactively to the accumulated run score and never to time-based coins.
