...
- The Hostinger release artifact uses Google Client's documented Composer cleanup hook to retain the OAuth2 wrapper while excluding tens of thousands of unrelated generated service clients; Google ID-token verification and the dependency audit remain covered by the PHP checks.
...
## Local Candidate 20260715-2 Native Pets, Zen Any HUD, and Muse QA
...
- All seven pet sheets are 640×64 with native 64×64 logical cells and binary runtime alpha. Muse uses the retained 2172×724 chroma/alpha pose master and one 64×48 two-layer wooden-floor habitat. Its front placement is intentionally narrow; every visible sprite cell keeps at least 2 px of transparent edge safety; the light wheat-brown ponytail remains readable in both sleep cells; and no limb is clipped.
- Muse runtime cells 5, 6, and 7 are byte-for-byte horizontal mirrors of cells 1, 2, and 3 after final resizing. The generated right-side anatomy is never shipped. A clean rebuild reproduced the Muse sprite/floor and every other pet runtime asset byte-for-byte.
- Exact confirmed lowercase `bloodyvlad` eligibility is derived by the server as `specialPetId: muse`, remains non-purchasable, and temporarily overrides rather than mutates the player's durable shop selection. Case variants and unconfirmed nicknames fail backend coverage. The existing exact Cyrillic `кокос` Mitsuri path remains unchanged.
- Zen displays **Any** beside a permanent colorful yin-yang swatch, independent of color-blind mode. The surrounding **Your color** field uses only the current theme panel/border plus an inset neutral shadow, applies no player-color glow, and disables its former 120 ms color transition. The menu overlay remains visible until the initial Zen render, preventing one stale Arcade-color frame.
- Local browser inspection at the 2022 iPhone SE CSS viewport of 375×667 showed the full Zen HUD, board, and streak meter without horizontal overflow (`scrollWidth = 375`). Computed Zen styles reported the Disco neutral border `rgba(218, 239, 244, 0.13)`, an inset-only neutral shadow, zero transition duration, and the intended six-color conic gradient. The browser console contained no warnings or errors.
- `npm run check` passed on the complete `20260715-2` graph with 160 JavaScript tests and 156 PHP assertions. Composer validation/audit, every PHP syntax check, deterministic asset reconstruction, exact runtime mirror comparison, and `git diff --check` passed.
- The curated production artifact must include only `assets/pets/*.png` and must exclude `assets/pets/sources/`; post-deployment smoke must confirm the Muse runtime assets load while a representative source-master URL is denied or missing.
- Physical iPhone Safari/PWA confirmation remains required for final likeness, ponytail legibility, wooden-floor alignment, yin-yang clarity, authenticated nickname activation, and service-worker cache replacement before this candidate is described as device-validated.

## Local Candidate 20260715-3 Menu Hints, Stable Pet Layout, and Admin Test Ownership QA

- This is pre-deployment candidate evidence. It does not by itself establish the active Hostinger deployment, and no physical-iPhone validation was performed.
- The main menu visibly removes **Ready to react?** while retaining one visually hidden accessible `PimPoPom` heading. Before the first Arcade Game Over it shows the exact three-line intro: **Tap your color**, **Become the fastest**, and **Collect rewards!**. A completed Zen practice run does not unlock or advance the post-run message state.
- After the first Arcade Game Over, the menu selects from the approved 26-line motivational pool. The selected line persists while navigating menu subviews, changes after each later Arcade Game Over without immediately repeating, and uses one of five contrast-aware colors plus a static one-to-three-degree tilt.
- Browser-runtime inspection at the 2022 iPhone SE CSS viewport of 375×667 measured the hint region at exactly 308×112 px with a 76 px pet-safe inset. Its top remained y=107.5 and the mode controls remained y=233.5 both with no pet and with a 64×144 px Foka scene present. The document had zero horizontal overflow and the browser console contained no warnings or errors.
- Repeated taps that resolve to the pet's already-current awake direction no longer restart the stepped turn animation. They refresh only the idle timer; sleeping, stopped, and settling pets still wake correctly, and an in-flight direction change still replaces the existing transition.
- Migration `013_grant_admin_test_cosmetics.sql` grants the original `migration-011` leaderboard administrator zero-price test ownership of all active shop pets and paid themes. It preserves real purchase metadata on duplicates and does not change coins, ledger history, achievements, selections, visibility, or nickname-only companions.
- `npm run check` passed on the complete `20260715-3` graph with 162 JavaScript tests and 163 PHP assertions; Composer validation/audit, every PHP syntax check, and `git diff --check` passed.
- Local evidence: `/tmp/speedytapper-20260715-3-menu-intro.png` and `/tmp/speedytapper-20260715-3-menu-pet.png`.
