...
final class AchievementService
{
    ...
    private function claimInTransaction(string $playerId, array $definition, bool $mayRetry): array
    {
        ...
        try {
            ...
            $coinBalance = (int) $player['coins'] + $rewardCoins;
    ...
    private function syncHistoricalEligibility(string $playerId, int $totalCoinsCollected): void
    {
        ...
        $godlike = $this->database->prepare(
            ...
            . 'WHERE player_id = :player_id AND godlike_count > 0 LIMIT 1'
        );
    ...
    {
        $statement = $this->database->prepare(
            'SELECT coins, total_coins_collected FROM players WHERE id = :player_id FOR UPDATE'
        );
