...
| `src/profile-client.js` | Same-origin Google profile, achievements, and leaderboard client |
| `server/src/` | PHP configuration, identity, sessions, exact score validation, achievement/coin accounting, and MySQL repositories |
...
5. The first API request applies pending migrations under a database advisory lock. Migration `004_clear_leaderboard_for_multiplier_scoring.sql` historically removed pre-multiplier rows once; migration `005_allow_multiple_leaderboard_results.sql` only replaces the one-result uniqueness constraint and preserves existing profiles, scores, coins, and completed-run history. Migration `007_player_achievements.sql` adds lifetime coin accounting and idempotent achievement claims without spending or resetting existing balances. The reviewed `php server/bin/migrate.php` remains available for an explicit maintenance run.
6. Purge only the SpeedyTapper website cache, then smoke-test HTTPS, build ID, app shell/service worker, `/api/health`, `/api/session`, denied configuration paths, Google sign-in/logout, nickname editing, Normal/Zen leaderboard submissions, achievement reads, and one claim/retry. Verify that a new run is added as its own result and that retrying the same run UUID or achievement claim remains idempotent.
...
- **3-min Zen** ends after exactly 180 seconds; mistakes are counted but lives are never removed and the HUD shows an infinity symbol. Its correct target has no expiry or response progress bar, remains visible through mistakes, and must be tapped before another correct target can be scheduled.
- Zen waits 1,000 ms before the first correct target. After each correct tap, the next delay moves halfway from its previous value toward that reaction time: `next delay = previous delay + 0.5 × (reaction − previous delay)`. Normal retains its programmed random quiet intervals.
- Correct taps award 100–1,000 points based on reaction time.
- Every independently spawned wrong-color decoy lives for 450–750 ms. A natural expiry is recorded as a dodge; it awards 550 points in Normal and no points in Zen. Decoy opportunities use wide randomized intervals and are approximately half as frequent as the preceding balance; even at maximum pressure the next opportunity waits at least 600 ms.
- The first four successful taps use one full-screen cell, then the board becomes 2×2.
- 0–10 seconds: one fixed player color and no wrong colors; a Normal target has a 1,000 ms lifetime.
- 10–20 seconds: one independent wrong-color decoy may appear between or during targets; the Normal target lifetime stays at 1,000 ms.
- 20–30 seconds: the Normal target lifetime eases gradually from 1,000 ms to 750 ms.
- 30–40 seconds: up to two independent decoys may overlap at random positions.
- At 40 seconds the board becomes 4×4, the Normal target lifetime resets to 1,000 ms, and decoy pressure eases back to one at a time.
- At 50 seconds the Normal target lifetime falls by 10 ms per correct tap toward a 200 ms floor. Every ten challenge taps can add another simultaneous decoy, up to six, and shortens both target and decoy quiet intervals without reducing the decoy-opportunity gap below 600 ms.
- A decoy never uses the player's current color. A target activation also reserves every cell that displayed a decoy immediately before that frame, so an expiring decoy cannot turn directly into the correct target. Correctly tapping the target, making a mistake, Normal target expiry, restart, or run end clears still-visible decoys without awarding dodges; a Zen mistake retains the correct target itself.
...
- In Normal, a single neutral-grey progress bar drains along the bottom of the **Your color** field during every active decision. Zen hides it because correct targets do not expire.
...
- The Achievements view tracks six server-backed milestones: complete Zen, complete Arcade (the internal `normal` mode), record a Godlike tap, collect five lifetime coins, score strictly more than 100,000 points in one run, and purchase a pet. Rewards are 1, 1, 1, 5, 5, and 10 coins respectively. An unlocked reward appears as a tappable green check; one successful claim credits the balance transactionally and greys the box. Locked and already-claimed rewards cannot be credited by repeated requests.
- Lifetime coins are monotonic eligibility data distinct from the spendable balance. Achievement rewards increase both values. Pet eligibility accepts only a recorded purchase and deliberately excludes free/easter-egg ownership; the check activates when the PHP pet-ownership migration is integrated.
...
The game engine is separate from the browser UI and has deterministic coverage for board progression, multiplier scoring and resets, empty-board penalties, independent overlapping decoys, mode-specific dodge scoring, decoy clearing, rounded speed ratings, gradual Normal pressure, Normal life loss, infinite Zen targets, adaptive Zen cadence, and exact three-minute Zen timing. Input-timing tests cover pointer timestamp normalization, presentation-anchored deadlines, pre-presentation input, and expiry/input race classification. Static UI tests cover the utility header, Google-only profile flow, coin balance, dedicated Settings/Achievements/Leaderboard navigation, achievement states, Game Over round trips, streak and result presentation, leaderboard speed distribution, distinct Classic/Disco materials, audio settings, runtime-music caching boundaries, and the unified release graph. PHP tests cover request guards, identity-safe payloads, exact mode-specific score accounting, idempotent coin rollover and achievement claims, eligibility criteria, best-player and exact-result ranking, top-five-plus-context windows, configuration isolation, and schema constraints. The retained Vercel leaderboard model remains covered for rollback compatibility.
