...
- The main menu gives Arcade a bright pink-red glow and Zen a light summer-leaf green glow, uses larger mode names, and places the Zen **No coins awarded** note below its name. Pet Shop and Themes share one compact 45%/45% feature row with a tight caption-to-current-selection gap.
...
- Beta sound uses standards-based Web Audio with an interactive-latency `AudioContext`. Every correct tap immediately plays the next half-second cue in a fixed 16-note Power Grid motif at native speed and a `0.375` base gain; misses, wrong colors, dodges, inactive taps, and unready buffers neither play nor delay a tap cue. Losing a life plays the separately predecoded original failure cue at the same `0.375` base gain, only while Sound FX is enabled, with at most one failure voice and two tap voices active. Retiring voices receive a short release. The Sound FX slider scales their shared output. There is no hum, `HTMLAudioElement`, or pace/reaction pitch shift.
- Music has an independent switch, defaults on, and remembers an explicit opt-out. After a trusted menu gesture it loops a 12-second, 80 BPM **Daylight Circuit** mix containing a restrained Power Grid melody. Starting Arcade or Zen switches to the matching clean backing-only version; correct-tap tones then come only from Sound FX. Results and Game Over are silent, while returning to the menu restores the melodic mix. Both variants use the same `0.42` base gain and independent Music slider. There is no adaptive stage, tempo change, track rotation, or Interactive Music behavior.
- Runtime audio is excluded from the offline app shell and always fetched without browser or service-worker caching. The lossless tap bank and failure cue ship directly; both encoded background variants retain lossless masters and provenance in `assets/audio/SOURCES.md`.
