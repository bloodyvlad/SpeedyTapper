...
| `assets/audio/` | Runtime audio, provenance, and the retained background master |
...
- A utility header gives the menu, Results, and Game Over views a compact SpeedyTapper logo, icon-only Leaderboard rank shortcut, and Profile shortcut. Arcade gameplay keeps compact Restart and Main menu controls above the HUD; Zen replaces both with one **End run** control. Both result views place compact square Restart and Main menu controls at the top and omit bottom navigation. `Copyright © 2026 OTC Software` anchors the dialog footer.
- The main menu gives Arcade a vivid red glow and Zen a calm light-green glow, uses larger mode names, and places the Zen **No coins awarded** note below its name. Pet Shop and Themes share one 45%/45% feature row.
- **Themes** is a dedicated menu view containing the Classic and Disco selector. Classic targets show the vivid palette immediately with no dark color-transition frame. Disco uses paler center-lit colors, clearly visible repeating black concrete, and lightly scratched plastic tile surfaces in both idle and lit states.
...
- Beta sound uses standards-based Web Audio with an interactive-latency `AudioContext`. Every correct tap immediately plays the next half-second cue in a fixed 16-note Power Grid motif at native speed and a `0.375` base gain; misses, wrong colors, dodges, inactive taps, and unready buffers neither play nor delay a tap cue. Losing a life plays the separately predecoded original failure cue at its retained `0.55` base gain, with at most one failure voice and two tap voices active. Retiring voices receive a short release. The Sound FX slider scales the shared output after that internal cue balance. There is no hum, `HTMLAudioElement`, or pace/reaction pitch shift.
- Music has an independent switch, defaults on, and remembers an explicit opt-out. During a run it plays one original 12-second, 80 BPM **Daylight Circuit** backing-only loop at `0.42` base gain, then fades out on Results, Game Over, or Main menu. The Music slider scales that base level independently. The F-sharp-major loop uses warm pads, restrained mid-bass, soft half-time tonal percussion, and no tap melody or bright lead. It has no adaptive stage, tempo change, track rotation, or Interactive Music behavior. It resumes only from a trusted gesture and stays silent outside gameplay.
- Runtime audio is excluded from the offline app shell and always fetched without browser or service-worker caching. The lossless tap bank and failure cue ship directly; the encoded background loop retains a cropped lossless master and provenance in `assets/audio/SOURCES.md`.
- Theme, accessibility, Sound FX, Music, and both volume preferences are stored on that device.
