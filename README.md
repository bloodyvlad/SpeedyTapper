...
- A five-step Speed streak meter sits below the board. Its large animated gradient fill shows progress, an explicit `x1`–`x5` label stays at the right, and the completed meter glows. The x1 track stays dark; x2 through x5 use green, blue, violet, and gold tier colors shared by the track background and multiplier label, with the animated progress gradient layered above that base. Godlike, Perfect, and Great taps advance it; every five qualifying taps unlocks the next score multiplier for subsequent taps, from 2× through a 5× cap. Good preserves the current meter and multiplier without advancing it. Only a mistake resets both before the next score, while decoy dodges remain neutral and their 550 points are never multiplied. A multiplier applies only to the tap currently being awarded: it never rescales points accumulated earlier in the run and never changes coin accrual.
