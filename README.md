...
- Each completed run asks for a name and can be submitted to a shared, mode-specific leaderboard retaining the best 1,000 results. Public leaderboard reads show the top five; immediately after submission, the result view can also show that run with up to two neighboring ranks on each side. After a valid entry, the name is remembered on the device and prefilled on future result screens as a form convenience only; it does not create a player profile, personal best, or local score history. Entries show survival or play time, taps, dodges, fastest reaction, and average reaction.
...
The game engine is separate from the browser UI and is covered by deterministic tests for board progression, scoring, empty-board penalties, dodge rewards, reaction statistics, rare adjacent decoys, gradual timing, Normal life loss, and Zen timing. Input-timing tests cover pointer timestamp normalization, presentation-anchored deadlines, pre-presentation input, and expiry/input race classification. Static UI tests cover dedicated Settings/Leaderboard navigation, Game Over round trips, gameplay shortcuts, same-mode restart flow, distinct Classic/Disco material treatments, the default-off Beta sound and Interactive Music settings, runtime-music caching boundaries, and the unified release graph. The audio controllers are tested for disabled behavior, decoded-buffer playback, click-safe PCM boundaries, legacy adaptive loop regions, Interactive state resolution, beat-scheduled authored bridges, deterministic tap-note order, bounded polyphony, variant replacement, interrupted fades, stale suspend/resume races, and resource cleanup. The leaderboard model is tested for validation, legacy-row compatibility, deterministic ranking, mode separation, reaction metrics, the 1,000-entry retention cap, and compact top-five-plus-context responses.
