...
| `server/src/` | PHP identity, CSRF/session handling, server-issued runs, proof replay, idempotent coin accounting, moderation, and MySQL repositories |
...
- A confirmed profile nickname that normalizes case-insensitively to `misha_boy` unlocks a cosmetic Misha pixel-cat easter egg. On the main menu he sits on a white climber with a light-blue top pouch; other non-game views keep Misha alone in the upper-right position. After five seconds without a non-game screen tap he lies down asleep, while any screen tap wakes him and turns him toward that side. During a run he perches above the Speed streak meter with the top of his head overlapping the board frame and turns toward each accepted board tap. Short or very narrow phone layouts keep Misha at 48px instead of the former 32px; roomier layouts use 64px. The easter egg changes no scoring, coins, ranking, or gameplay rule.
