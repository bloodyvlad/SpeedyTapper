...
| `server/src/` | PHP configuration, identity, sessions, exact score validation, idempotent run/coin accounting, pet ownership, and MySQL repositories |
...
5. The first API request applies pending migrations under a database advisory lock. Migration `004_clear_leaderboard_for_multiplier_scoring.sql` historically removed pre-multiplier rows once; migration `005_allow_multiple_leaderboard_results.sql` only replaces the one-result uniqueness constraint; migration `006_pet_shop.sql` adds the pet catalog, durable ownership, and equipped-pet selection while granting Misha once to already-confirmed legacy `misha_boy` profiles. The reviewed `php server/bin/migrate.php` remains available for an explicit maintenance run.
...
- Every independently spawned wrong-color decoy lives for 450–750 ms. Letting it expire naturally is a dodge worth 550 points. Decoy opportunities use wide randomized intervals and are approximately half as frequent as the preceding balance; even at maximum pressure the next opportunity waits at least 600 ms.
...
- Interactive backing uses twelve pre-recorded four-bar states: a 100 BPM opening, richer 104–112 BPM 2×2 development, a substantially richer 112 BPM 4×4 reset, and 120–168 BPM challenge states. Each game phase/challenge tier maps directly to one authored state; measured reaction milliseconds never select music, although successful taps advance the programmed challenge tiers. Adjacent changes wait no longer than the next beat to enter a dedicated bridge before the destination loop; the tap note itself is never quantized. As the audible state grows richer, the existing five-degree note bank is lifted diatonically with octave wrapping up to 2× playback rate, so tap tones rise with programmed game pace. Reverse bridges remain available for future pace reductions.
...
- **Pet Shop** appears immediately above Settings. Its stable catalog is Foka the baby seal on an ice floe for 10 coins, Kesha the green-yellow parrot on a perch for 20, Tauta the black-white border collie on a cozy brown bed for 50, Misha the grey cat on his white climber and light-blue pouch for 100, and Pancake for 500. Tapping an icon plays its sprite preview. An unowned pet exposes only **Buy**, which debits the exact server-side price and equips immediately; an owned unequipped pet exposes only **Change**, which never charges again; the selected card shows a non-button **Equipped** state. Signed-out players can browse previews but cannot mutate ownership.
- Pet ownership and selection are durable profile data. The server serializes purchases by locking the player balance, guards the debit against insufficient funds, inserts ownership, and changes the equipped pet in one transaction. Existing confirmed `misha_boy` profiles receive a one-time free Misha migration and keep him selected when they had no prior choice; renaming to `misha_boy` after that migration is not a free-unlock path. A narrow client fallback recognizes the nickname only when talking to an older API response that has no pet fields.
- The equipped companion stays under the Profile shortcut at the established upper-right position on every non-game view and moves above the Speed streak meter during a run, with its ears or top edge slightly overlapping the non-interactive board frame. Home props are shown on the main menu and in shop previews, not on other views or over the game board; Pancake retains only a two-pixel side-view glow line. The four animal pets use full-left, half-left, front, half-right, and full-right poses from the last accepted tap. Misha and Tauta use an intermediate rising/crouched frame around sleep, Foka always remains lying, Kesha sleeps upside-down, and Pancake uses only its source orientation plus a horizontal mirror for left-facing taps, dances after interaction, and rests in the supplied down pose after five seconds without taps. The main and game companions remain 64px even on the short iPhone SE viewport so they do not regress to the previously-too-small compact rendering.
- Leaderboard rows show the player's currently equipped pet as a 32px profile picture. Changing pets updates the portrait for that profile's existing rows; the pet is not snapshotted into an individual historical run.
- A signed-in completed run submits automatically with a stable run UUID. Migration `004` historically cleared pre-multiplier scores; migration `005` preserves current data while allowing every accepted run to become an immutable leaderboard row. Reads return the top five plus either the profile's best result or the just-submitted result with up to two neighboring ranks on each side. Entries include duration, taps, dodges, fastest/average reaction, and all four speed-rating counts. The run ledger still prevents a retry from inserting or rewarding the same UUID twice.
- The utility header shows the signed-in coin balance immediately left of Leaderboard. Every cumulative completed minute earns one coin; sub-minute time carries across accepted runs, and an idempotent completed-run ledger prevents retrying the same run from awarding twice. Coins can buy durable pets but this browser-authoritative prototype currency remains farmable, cannot be purchased or redeemed, and has no real-money value.
...
The game engine is separate from the browser UI and has deterministic coverage for board progression, multiplier scoring and resets, empty-board penalties, independent overlapping decoys, natural-expiry dodge rewards, decoy clearing, rounded speed ratings, gradual pressure, Arcade life loss, and exact three-minute Zen timing. Input-timing tests cover pointer timestamp normalization, presentation-anchored deadlines, pre-presentation input, and expiry/input race classification. Pet tests cover the stable catalog, legacy migration fallback, five directional bands, intermediate sleep/wake pose, stale timer protection, main-menu habitats, Pancake mirroring/rest, and buy/change client payloads. Static UI tests cover the utility header, Google-only profile flow, coin balance, dedicated Pet Shop/Settings/Leaderboard navigation, pet avatars, Game Over round trips, streak and result presentation, distinct Classic/Disco materials, audio settings, runtime-music caching boundaries, sprite dimensions, offline assets, and the unified release graph. PHP tests cover request guards, identity-safe payloads, exact multiplier score accounting, idempotent coin rollover, atomic pet-selection invariants, pet schema/migration wiring, equipped-pet leaderboard joins, ranking windows, configuration isolation, and schema constraints. The retained Vercel leaderboard model remains covered for rollback compatibility.
