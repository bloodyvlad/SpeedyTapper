...
- A utility header gives the menu, Results, and Game Over views a compact three-gradient **PimPoPom** wordmark, icon-only Leaderboard rank shortcut, and Profile shortcut. Arcade gameplay keeps compact Restart and Main menu controls above the HUD; Zen replaces both with one **End run** control. Both result views place compact square Restart and Main menu controls at the top and omit bottom navigation. `Copyright © 2026 OTC Software` anchors the dialog footer.
