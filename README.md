...
6. Purge only the SpeedyTapper website cache, then smoke-test HTTPS, build ID, app shell/service worker, `/api/health`, `/api/session`, denied configuration paths, Google sign-in/logout, nickname editing, and Normal/Zen leaderboard submissions. Verify that a new run is added as its own result and that retrying the same run UUID remains idempotent.
