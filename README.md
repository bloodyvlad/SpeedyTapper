...
- A five-step Speed streak meter sits below the board. Its large animated gradient fill shows progress, an explicit `x1`–`x5` label stays at the right, and the completed meter glows. Godlike and Perfect taps advance it; every five qualifying taps unlocks the next score multiplier for subsequent taps, from 2× through a 5× cap. Great and Good preserve the current meter and multiplier without advancing it. Only a mistake resets both before the next score, while decoy dodges remain neutral and their 550 points are never multiplied. A multiplier applies only to the tap currently being awarded: it never rescales points accumulated earlier in the run and never changes coin accrual.
