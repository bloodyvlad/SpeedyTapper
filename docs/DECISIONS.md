...
Amendment (2026-07-20): Advance the current release identifier to `20260720-1` for the provider-neutral identity backend and add it to the same explicit compatibility set. The gameplay ruleset and proof version are unchanged, and `20260718-1` through `20260719-3` remain accepted so the installed iOS build and preceding PWA releases continue ranked play.

Amendment (2026-07-27): Advance the current release identifier to `20260727-1` for the guarded internal-alpha reset release. Explicitly retain `20260720-1` and `20260725-1` alongside the earlier compatible IDs because gameplay semantics, `reaction-proof-v2`, and proof version 1 remain unchanged.
...
## D-068 — Add Apple primary identity and link-only Game Center without implicit account merging

- Date: 2026-07-20
- Status: Accepted

Context: The native iOS client needs Sign in with Apple and Game Center while existing Google profiles already own durable UUID-bound wallets, purchased value, StoreKit account tokens, scores, achievements, pets, and themes. Treating a new provider response as a new player by default can duplicate wallets; guessing that two unrelated provider subjects represent the same human can instead transfer value to the wrong account. Game Center's signed identity can bind a game account, but it is not suitable as the sole recovery-capable primary identity. Apple also requires server-side authorization-code handling and revocation when an Apple-linked account is deleted.

Decision: Keep the internal player UUID as the only owner of profile, wallet, StoreKit, leaderboard, achievement, and cosmetic state. Migration `018_primary_identities_and_game_center.sql` creates `player_identities`, backfills every existing Google digest without changing its UUID, makes the legacy Google column nullable for Apple-created profiles, and enforces one subject per provider per player plus one player per provider subject. Hash every external identity with a provider-domain separator and never store raw Google/Apple subjects, identity tokens, email/relay-email claims, provider display names, or passwords.

Treat Google and Apple as primary authentication providers. Require explicit `login`, `register`, `link`, or `reauth` intent for new native flows; retain omitted Google `login_or_register` only for the current browser compatibility path. An unknown `login` returns `409` and must not be silently retried as registration. Linking requires an authenticated intended UUID and recent Google/Apple primary authentication. Never merge profiles, move wallets, or infer sameness from email, Apple relay email, nickname, device, StoreKit `appAccountToken`, Game Center, score, or purchase history. A conflicting binding returns `409` and leaves both UUIDs unchanged.

Use a single-use session nonce/state challenge for Sign in with Apple. Verify the client ID token against Apple's fixed JWKS origin, RS256 signature, exact issuer/audience/nonce/subject/timing, exchange the one-time authorization code with a short-lived ES256 client secret, and independently verify the returned identity token has the same identity. Use a distinct Sign in with Apple key rather than the StoreKit IAP key. Retain the Apple refresh token only as AES-256-GCM ciphertext under a separate stable private secret; commit a new Apple identity/link and its revocation material atomically. Before deleting an Apple-bound profile, revoke that authorization at Apple; failure leaves the local profile intact for retry.

Treat Game Center as a link-only secondary binding that cannot register, log in, create a wallet, or satisfy recent primary reauthentication. Verify the signed non-Apple-Arcade tuple `teamPlayerID || bundleID || UInt64BE(timestamp) || salt`, accept certificates only from the allowlisted Apple HTTPS origin, validate the reviewed DigiCert chain and Apple code-signing leaf constraints, bind proof freshness to a single-use server challenge, and retain only a domain-separated hash of `teamPlayerID`. Do not accept or store unsigned `gamePlayerID`. One Game Center identity can map to only one internal UUID, and each UUID can have at most one Game Center binding.

Expose only provider-binding booleans in session/profile state. The current browser remains Google-first until a separate UI task consumes these endpoints; the compatibility backend continues to use same-origin cookie sessions and CSRF rather than inventing an undocumented bearer-token flow. This decision supersedes the Google-only identity and recent-Google-reauth scopes of D-017, D-043, D-046, and D-065; their ranking, moderation, paid-value, and deletion-evidence rules otherwise remain in force.

Consequences: A verified Google or Apple login resolves the same durable UUID after explicit linking, so adding a provider does not duplicate or transfer a wallet. Game Center can mirror identity/achievement context without becoming an unrecoverable login mechanism. The backend cannot know that an unlinked Google subject and Apple subject belong to one person, so the native UI must attempt login first and guide an existing player through explicit linking rather than automatic registration. Apple sign-in is not production-ready merely because this backend exists: the separate Sign in with Apple key/configuration, native nonce/state/code client flow, Apple Sandbox/device tests, Game Center live certificate flow, deletion/revocation exercise, migration, and deployment smoke tests remain required.

Revisit when: Apple changes token, revocation, GameKit signing, or certificate-chain requirements; Game Center gains a documented recovery-grade server login contract; the product introduces deliberate account-merge support with high-assurance proof and paid-value migration; or the compatibility API moves from cookie/CSRF sessions to a separately designed native token session.

## D-069 — Mirror authoritative progress to Game Center through a durable server outbox

- Date: 2026-07-25
- Status: Accepted

Context: PHP already verifies Arcade proofs, stores immutable leaderboard results, and decides achievement unlocks, while the native client already authenticates `GKLocalPlayer`. Neither side publishes those authoritative outcomes to Apple. Direct client reporting would let a modified app create a Game Center-only score or achievement that PHP never accepted, and making a ranked completion wait for Apple would turn an optional mirror into a gameplay dependency. Apple's App Store Connect submission API also requires a scoped `gamePlayerID`, while the ordinary non-Apple-Arcade GameKit identity signature covers `teamPlayerID`, bundle ID, timestamp, and salt—not the accompanying `gamePlayerID`.

Decision: Keep Google or Apple primary authentication and the internal player UUID as the account authority. Keep Game Center link-only: it cannot register, log in, own a wallet, merge profiles, or satisfy recent primary reauthentication. After a fresh server challenge, recent primary authentication, verified signed `teamPlayerID`, and a native `scopedIDsArePersistent()` check, allow the native client to explicitly request publication and accompany the proof with its `gamePlayerID`. Bind that pair once and one-to-one. Store only a domain-separated uniqueness hash and AES-256-GCM ciphertext/IV/tag under a dedicated backend secret; never return, log, or persist the raw ID elsewhere. Describe this as a **client-asserted association**, not a Game Center-signature-verified mapping. This supersedes D-068 only where it categorically prohibited accepting or retaining `gamePlayerID`; all no-merge and link-only rules remain.

Use a distinct App Store Connect API key and short-lived ES256 JWT against `api.appstoreconnect.apple.com`; do not reuse the StoreKit Server API client, StoreKit IAP key, or Sign in with Apple key by assumption. Configure `preReleased` only on the server: true for the prerelease/TestFlight lane and false for the released lane. The fixed leaderboard `com.otcsoftware.pimpopom.arcade.verified` represents the player's all-time best verified Arcade score across PHP seasons. Map the five existing achievement IDs to their fixed Apple vendor identifiers and report 100 percent at first unlock, independently of whether its coin reward was claimed.

Migration `019_game_center_server_publication.sql` extends the binding and adds a coalescing desired-state outbox. Enqueue within the same transaction as: every verified Arcade result (then derive the best from verified-only normal rows), every first achievement insert, a successful initial link/backfill, and a moderation transition that can change the best or newly approve run-derived achievements. Retired or unknown historical achievement rows are skipped during Apple backfill. Never enqueue Zen, legacy, review, quarantined, or deleted rows. Process Apple outside the game transaction through a bounded Hostinger cron worker with a lane lock, row lease, desired revision, current-authority recheck, short timeout, and retry/backoff. Hold a per-player advisory lock across the bounded Apple call and acquire that same lock before publication disable or account deletion, so either privacy operation cannot return while an already-prepared write can still escape. A newer desired revision invalidates an in-flight completion, and Apple failure never rolls back PimPoPom progress. Nonretryable responses and twelve failed attempts enter operator hold until an explicit recovery command; ordinary gameplay must not revive them. Account deletion cascades the encrypted binding and outbox. Explicitly disabling publication preserves the link but cancels unsent jobs.

Apple documents this server endpoint as overwriting the player's existing score, so moderation submits a lower replacement when another verified best remains. If no verified score remains, retain an operator-visible `needs_reset` state rather than inventing a zero score or claiming synchronization; Apple's documented submission surface has no per-player delete/reset operation. Achievements remain durable under the existing moderation policy. Revalidate every leased job before network delivery so a quarantined pending score cannot escape.

Consequences: PHP remains the only authority for leaderboard and achievement progress, while Apple becomes an eventually consistent mirror. Game runs remain available when Apple is slow or unavailable, and duplicate/timeout retries are locally coalesced even though Apple exposes no request idempotency key. A live TestFlight link, real App Store Connect credential, Hostinger cron, and Apple dashboard observation are still required before calling the mirror production-validated. App Attest may raise the cost of fabricating the client association but cannot make the GameKit signature cover `gamePlayerID`.

Revisit when: Apple documents a cryptographic `teamPlayerID`↔`gamePlayerID` mapping, accepts signed `teamPlayerID` as `scopedPlayerId`, adds a supported player-state reset/readback or request-idempotency API, App Attest becomes part of the native trust model, prerelease and released lanes must be published concurrently for the same player, or each PHP season receives its own Apple leaderboard.

## D-070 — Perform one guarded internal-alpha account reset while retaining payment tombstones

- Date: 2026-07-27
- Status: Accepted

Context: Historical internal profiles predate the final provider-linking and Game Center publication model. Their conflicting bindings prevent a trustworthy clean-room validation of Google/Apple linking, persistent Game Center identity, leaderboard publication, achievements, and paid-value behavior. The product owner explicitly authorized deleting the internal-alpha players and their player-visible scores, purchases, achievements, and cosmetics. A literal deletion of all StoreKit records would, however, remove lifetime transaction idempotency and the evidence needed to reconcile a later Apple refund, reversal, revocation, or Family Sharing event.

Decision: Apply `020_reset_internal_alpha_player_data.sql` once under an operator-controlled maintenance window. Take and verify a database backup; stop the Game Center publication and StoreKit reconciliation crons; deploy an artifact-only maintenance marker that returns `503` before session or database work; and invoke the migration from a private host CLI rather than through a public first request. The migration must acquire the prerelease/production Game Center and Sandbox/Production StoreKit advisory locks, claim a durable reset marker inside the destructive transaction, and delete every live profile, provider identity, session, role, leaderboard result, run/proof, wallet ledger, achievement, cosmetic, StoreKit binding, Game Center binding, and pending publication job.

Retain only detached payment-settlement and idempotency evidence: signed StoreKit transactions and observations, notification UUIDs/payload evidence, purchased lots, entitlement/refund records, purchased spend allocations, Family Sharing tombstones, and reconciliation cursors. Remove their live player/app-transaction associations and replace remaining player-correlatable references with the existing keyed account pseudonym. Tombstone even previously uncredited transactions so an old signed transaction cannot be submitted to a newly registered profile after the reset. Catalogs, seasons, schema history, and the migration marker also remain. A rerun after a successful reset must not delete profiles created afterward.

This is an operational alpha reset, not a substitute for the normal per-user deletion endpoint: it does not perform one Apple authorization-revocation request per profile. Apple-hosted Game Center test data and Sandbox purchase history are separate systems and require their own App Store Connect/tester reset operations. After service resumes, the owner must register or log in again, explicitly link the second primary provider, freshly verify the current GameKit player, and regrant any required database administrator role.
...
## D-073 — Auto-assign the active Game Center player to the current authenticated profile

- Date: 2026-07-28
- Status: Accepted

Context: The iOS app already has an authenticated PimPoPom session and an authenticated `GKLocalPlayer`. Requiring another recent Google/Apple reauthentication and explicit connect decision every time the active Game Center player differs creates needless friction, while refusing an old one-to-one binding with `409` can leave the current signed-in profile unable to mirror its authoritative progress. Game Center remains an optional mirror and is not an account authority.

Decision: After a fresh session-bound challenge and valid GameKit proof, require `publish: true` plus the persistent scoped IDs and make the submitted team/game pair belong to the currently authenticated PimPoPom profile. Remove recent-primary-reauthentication only from the Game Center challenge and proof routes. Keep session authentication, CSRF, challenge expiry/single use, certificate/signature/bundle/timestamp validation, assertion replay protection, strict request fields, and all recent-authentication requirements for primary-provider linking, publication disable, account deletion, StoreKit, and administration.

Implement this as Game-Center-only **current-profile-wins reassignment**, never as profile merging. Discover the current binding plus team/game owners; acquire every affected per-player publication advisory lock in sorted UUID order; re-read player, binding, and outbox rows under one transaction; retry a bounded number of times if the locked set is incomplete; and release locks in reverse order. An identical pair only refreshes verification/publication and non-destructively backfills, preserving delivered jobs. A destination change revision-cancels every affected prerelease and production outbox row, clears all lease/delivery/error evidence, removes displaced/current-old bindings, encrypts the submitted game ID anew with the final player/team associated data, creates the current binding, and backfills only the current profile's server-authoritative state. Moderation, worker delivery, publication disable, reassignment, and account deletion share the same per-player lock order.

This supersedes only D-068's Game Center ownership-conflict refusal and D-069's recent-reauthentication, explicit-connect-consent, and bind-once portions. Their no-profile-merge, link-only, encryption, privacy, server-authority, and client-asserted `gamePlayerID` rules remain. Game Center cannot register, log in, recover a wallet, authorize StoreKit value, or move scores, purchases, identities, entitlements, pets, themes, achievements, or currency between internal UUIDs.

Consequences: The active signed-in profile can recover from stale or conflicting Game Center bindings without duplicating or transferring its account data. A stale publication lease cannot acknowledge or retry after reassignment because its revision/token no longer matches. Apple offers no supported per-player removal of an already delivered score or achievement, so reassignment stops future writes to the old destination and publishes current authority to the new one but cannot erase prior Apple history. Apple's ordinary GameKit proof signs `teamPlayerID`, not the accompanying `gamePlayerID`; PHP therefore continues to describe that pair as client-asserted. `scopedIDsArePersistent()` is observable by GameKit on iOS but is not independently derivable from the retained PHP body or signature, so the native check remains part of the boundary.

Revisit when: Apple signs the team/game pairing, exposes a supported player-history reset, makes Game Center a recovery-grade primary identity, or the product designs an explicit high-assurance profile merge.

## D-074 — Make confirmed public player names whitespace-free and unique

- Date: 2026-07-28
- Status: Accepted

Context: New profiles historically received a spaced `Player 1234` placeholder and the nickname normalizer collapsed rather than rejected whitespace. A TestFlight player believed that placeholder was already a saved public name; the first ranked result was absent until a no-space name was explicitly saved. The public namespace also had no database uniqueness constraint, so an advisory client lookup alone could not prevent two devices from claiming the same name.

Decision: Keep Unicode names and NFKC normalization, retain the 20-character limit, and reject every Unicode separator or whitespace character rather than trimming or collapsing it. Generate new unconfirmed placeholders as stable no-space `Player` values derived from the internal UUID. Migration `021_unique_player_nicknames.sql` marks legacy confirmed whitespace names unconfirmed, replaces every unconfirmed legacy placeholder without deleting its profile or progress, and creates a nullable generated key for confirmed names with a unique database index. The index inherits `utf8mb4_unicode_ci`, so uniqueness is case- and accent-insensitive. If historical valid confirmed names already collide under that comparison, the migration stops for explicit operator resolution; it never chooses an owner, renames a confirmed claimant, merges profiles, or deletes results automatically.

Add authenticated, same-origin, CSRF-protected `POST /api/profile/nickname/availability`, accepting only `nickname`. It applies the exact save-time normalizer, excludes the current profile, returns the normalized candidate plus a Boolean, and discloses no owner data. Treat this response as advisory. The database unique index remains authoritative, and `PATCH /api/profile` maps a duplicate-key race to `409` with **This player name is already taken.**
