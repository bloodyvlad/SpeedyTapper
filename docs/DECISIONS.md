...
## D-035 — Make pets durable spendable cosmetics

- Date: 2026-07-13
- Status: Accepted

Context: The Misha preview validated a companion in the menu and reaction layout. The accepted follow-up now needs five selectable pets, server-side coin spending, permanent ownership, richer left/right poses, shop previews, and leaderboard portraits. The 48px compact Misha treatment from D-034 also remained too small on the short iPhone SE Safari viewport. Pancake must follow the supplied recording rather than a newly interpreted drawing, and its platform must read edge-on as a glow line rather than a floor tile.

Decision: Add a Pet Shop above Settings with the stable catalog `foka/10`, `kesha/20`, `tauta/50`, `misha/100`, and `pancake/500`. Store catalog rows, profile ownership, and the single equipped choice in MySQL. Use one same-origin `POST /api/pets/select` operation for both player actions: an unowned pet atomically locks the player, checks and debits its exact coin price, creates ownership, and equips immediately; an owned pet changes selection without another charge. The shop exposes only **Buy** for an unowned card and **Change** for an owned unequipped card, with a noninteractive Equipped state for the current pet. Profiles expose `ownedPetIds` and `equippedPetId`; leaderboard reads join the current selection and publish a nullable `petId`, so Change updates that profile's portrait on existing rows rather than snapshotting a cosmetic into each run.

Migration `006_pet_shop.sql` grants and equips Misha once, for zero coins, to profiles that were already confirmed as `misha_boy` when the migration runs. New nickname changes do not grant ownership. During cached old-client/old-API overlap only, the browser may retain the nickname fallback when both persisted pet fields are absent; persisted selection is otherwise authoritative. This replaces D-033's nickname-only entitlement and no-shop rule. It also supersedes D-034's 48px compact rendering and companion-specific behavior while retaining D-034's accepted player-facing Arcade rename and `normal` protocol identifier.

Use ten-cell 320×32 sprite sheets with persistent full-left, half-left, front, half-right, and full-right states for the four directional animals, plus an intermediate settle/rise cell and an idle endpoint. Misha and Tauta visibly pass through the intermediate crouched/half-standing frame when lying down or waking; Foka never stands; Kesha's idle endpoint hangs upside-down. Main-menu homes use two-layer prop sheets, while other non-game views keep the pet alone at the upper-right anchor and gameplay places it above the Speed streak meter without a home prop. Keep the visible menu/game sprite at 64px on short and narrow phones as well as roomier layouts, preserve pointer transparency, and leave the small top edge overlapping only the non-interactive board frame.

Pancake is the exception to the directional sheet semantics: its artwork is extracted from the user-supplied recording, right-facing is the source orientation, left-facing is only a horizontal mirror, and the supplied down pose is its stopped/resting state. It dances after interaction and stops after five seconds without taps. Draw no perspective floor or bitmap tile underneath it; use only a two-pixel horizontal glowing line, including in the shop preview. Keep all pets cosmetic: they affect neither game rules nor scoring. Coins remain browser-farmable prototype currency with no purchase, redemption, or real-money value.

Consequences: The PHP generation gains durable spendable cosmetics and a schema-backed selection invariant: a selected pet must already be owned. Player-row locking serializes pet purchases with completed-run coin awards, retries cannot charge the same pet twice, and exact client/server catalog parity requires release tests. Every new sprite and prop becomes part of the versioned offline app shell. The supplied Pancake source and generated animal art need retained provenance, and physical iPhone Safari/installed-PWA checks remain required for touch direction, short-screen fit, animation timing, offline upgrades, and readable board overlap before production validation.

Revisit when: Pets gain gameplay effects, sale/refund/gifting, randomized acquisition, real-money value, server-authoritative earning, historical run-time cosmetics, additional animation states, accessibility controls beyond reduced motion, or distribution rights require replacing any supplied visual source.
