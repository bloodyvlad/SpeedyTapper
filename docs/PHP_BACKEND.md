...
- `POST /api/app-store/notifications/v2` accepts exactly `{ "signedPayload": "APPLE_NOTIFICATION_V2_JWS" }`. It does not use a player session; the nested Apple signatures, app identity, retained notification UUID, and payload hash provide authentication and idempotency. `ONE_TIME_CHARGE`, `REFUND`, `REVOKE`, and `REFUND_REVERSED` are processed; unsupported events and `CONSUMPTION_REQUEST` are retained/ignored without inventing a policy response.
...
App Store Server Notifications V2 are the primary asynchronous refund/reversal path. Notification UUID plus payload hash and transaction ID plus immutable signed fields make retries idempotent and conflicting replays fail closed. Transaction JWS observations retain their own maximum signed date, while lifecycle changes use the separate outer-notification signed-date watermark when available, so an older refund or reversal cannot undo a newer transition merely because Apple re-signed the nested transaction later.

Run `php server/bin/reconcile-storekit.php --limit=100` from a bounded cron or operator job after configuring App Store Server API credentials. The command takes the `speedytapper-storekit-reconcile` MySQL advisory lock, replays notification history with a one-minute overlap (or the preceding 24 hours on its first run), follows at most 20 history pages, and rechecks a transaction-ID-ordered batch of retained transactions. The default transaction limit is 100 and the maximum accepted limit is 500. It stores its history timestamp, transaction cursor, and last error in `storekit_reconciliation_state`; a failed/incomplete pass does not advance successful state. This repair loop complements notifications and client delivery rather than replacing JWS verification.
