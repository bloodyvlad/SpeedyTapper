...
Migration `018_primary_identities_and_game_center.sql` introduces a provider-neutral identity layer without replacing or merging internal player UUIDs. It backfills every existing `players.google_subject_hash` into `player_identities` as `provider = 'google'`, preserving the exact UUID, wallet, StoreKit binding, scores, cosmetics, and achievements. New Google and Apple identities use `SHA-256(provider + "\\0" + subject)`. One verified provider subject can own only one internal UUID, and one UUID can have at most one subject for each primary provider. `player_game_center_bindings` retains one row per profile plus unique team/game scoped hashes; those uniqueness constraints serialize current-profile-wins reassignment rather than authorizing a profile merge. Only hashes are stored; raw Google subjects, Apple subjects, Game Center identifiers, provider tokens, email addresses, relay email addresses, and display names are not persisted.

Game Center is a link-only secondary binding. It cannot register a profile, create or recover a wallet, satisfy recent primary reauthentication, or log in independently. Once a PimPoPom session is authenticated, the native app may automatically request a single-use challenge and submit a fresh proof with `publish: true`. The server verifies GameKit's signed `teamPlayerID || bundleID || UInt64BE(timestamp) || salt` tuple with the leaf certificate from the exact allowlisted Apple HTTPS origin, a bundled reviewed DigiCert intermediate/root chain, Apple subject and code-signing constraints, freshness relative to that challenge, and a database replay digest. It then atomically assigns the submitted team/game pair to the current profile, displacing only stale Game Center bindings and publication destinations. It never merges or moves profile, provider, wallet, StoreKit, score, achievement, pet, theme, or entitlement data. PHP retains only domain hashes plus newly generated authenticated ciphertext for the client-asserted persistent `gamePlayerID`.
...
- `POST /api/profile/game-center/challenge` accepts `{}` in any authenticated PimPoPom session; it deliberately does not require another recent primary reauthentication.
- `POST /api/profile/game-center` accepts exactly `challengeId`, `teamPlayerId`, `gamePlayerId`, `publish: true`, `publicKeyUrl`, base64 `signature`, base64 `salt`, and integer millisecond `timestamp`.

A signed-out native client must try `login` first. A `409` means that provider subject is not linked; it must not be silently retried as `register`. The UI should offer either sign-in through an existing method followed by explicit linking, or an explicit new-profile registration. The backend can enforce that one external identity never owns two wallets, but it cannot infer that unrelated Google and Apple subjects are the same human. It therefore never merges profiles or moves a wallet based on email, Apple relay email, nickname, device, StoreKit `appAccountToken`, or Game Center data. Conflicting primary-provider links return `409` and leave both UUIDs unchanged; a valid Game Center auto-link is the deliberately narrower current-profile-wins exception described below and moves only the secondary binding/publication destination.
...
If Apple reports that the selected key does not allow player-score requests, place the replacement Admin/App Manager team key at `~/.config/speedytapper/AuthKey_KEYID.p8` with mode `0600`, then run `php server/bin/configure-game-center-publisher-key.php --key-id=KEYID`. The guarded command derives the private path rather than accepting an arbitrary one, rejects explicit-path or Game Center environment overrides, an unsafe configuration directory, symlinked or group/world-readable private files, non-P-256 material, and reuse of StoreKit or Sign in with Apple keys. It creates the staged configuration exclusively with owner-only permissions, validates the complete staged runtime configuration, and atomically changes only the Game Center key ID/path. It never prints configuration secrets. Do not implement automatic fallback across several Apple credentials.
...
Game Center is link-only. After the PimPoPom PHP session and `GKLocalPlayer` are authenticated, confirm `scopedIDsArePersistent()` on iOS, request a challenge with `{}`, obtain a fresh GameKit identity signature, then submit:
...
The signed proof must be fresh and no older than the current challenge (with a 30-second clock tolerance). Both routes require the authenticated cookie session and same-origin CSRF, but deliberately do not require another recent Google/Apple reauthentication. The server signature verification covers `teamPlayerID`, bundle ID, timestamp, and salt. For a normal non-Apple-Arcade game it does **not** cover `gamePlayerID`; that accompanying value remains a client assertion used only because Apple's server submission API requires `scopedPlayerId`. PHP cannot independently derive GameKit's persistence flag from this body. It domain-hashes both IDs, encrypts the game ID with fresh AES-GCM player/team associated data, and never returns or logs either raw value.

`publish` must be exactly `true` and `gamePlayerId` is required. A valid request applies current-profile-wins only to the Game Center association: it takes the team/game pair from stale internal bindings, revision-cancels every affected lane/destination, and backfills the current profile. The same current pair is idempotent and preserves successful delivery evidence. No profile, primary identity, wallet, StoreKit value, score, achievement, cosmetic, or entitlement is merged or transferred, and no route permits Game Center-only login or registration. Apple has no supported per-player removal for an already delivered score or achievement, so changing destinations stops future writes to the old player but cannot erase their Apple-hosted history.

### `DELETE /api/profile/game-center/publication`

After recent primary authentication, body `{ "confirm": true }` disables future Apple mirroring while retaining the verified Game Center identity binding. Pending, leased, retry, and reset-needed jobs are revision-cancelled. A per-player advisory lock serializes this operation with the bounded Apple request, so success cannot race an already-prepared publication. Re-enabling uses the same authenticated auto-link flow and a fresh challenge.
...
The native API is rooted at `/api/mobile/v1/multiplayer`:
